        ...
        <div class="row">
            ...
            <div class="form-group col-md-3 col-sm-2">
                ...
                <input class="form-control" name="address" value="{{old('address')}}"
                       placeholder="Адрес офиса компании">
                <p class="help-block">*обязательное поле</p>
            </div>
            ...
            <div class="form-group col-md-3 col-sm-2">
                ...
                <div>
                    <select class="form-control" name="status">
                        <option selected disabled @if (old('status') == '') selected="selected" @endif>Выберите статус
                            взаимодействия
                        </option>
                        <option value="Партнеры" @if (old('status') == 'Партнеры') selected="selected" @endif>Партнеры
                        </option>
                        <option value="Ведется диалог"
                                @if (old('status') == 'Ведется диалог') selected="selected" @endif>Ведется диалог
                        </option>
                        <option value="Потенциальные" @if (old('status') == 'Потенциальные') selected="selected" @endif>
                            Потенциальные
                        </option>
                        <option value="Неотслеживаемые"
                                @if (old('status') == 'Неотслеживаемые') selected="selected" @endif>Неотслеживаемые
                        </option>
                    </select>
                </div>
                <p class="help-block">*обязательное поле</p>
            ...
            <div class="form-group col-md-3 col-sm-2">
                ...
                <div>
                    <select class="form-control" name="type">
                        <option selected disabled @if (old('type') == '') selected="selected" @endif>Выберите тип
                            сотрудничества
                        </option>
                        <option value="Трудоустройство"
                                @if (old('type') == 'Трудоустройство') selected="selected" @endif>Трудоустройство
                        </option>
                        <option value="Информационное" @if (old('type') == 'Информационное') selected="selected" @endif>
                            Информационное
                        </option>
                        <option value="партнерство" @if (old('type') == 'партнерство') selected="selected" @endif>
                            партнерство
                        </option>
                        <option value="Проведение мероприятий"
                                @if (old('type') == 'Проведение мероприятий') selected="selected" @endif>Проведение
                            мероприятий
                        </option>
                        <option value="Отсутствует" @if (old('type') == 'Отсутствует') selected="selected" @endif>
                            Отсутствует
                        </option>
                    </select>
                </div>
